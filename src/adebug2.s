...
treat_instruction3:	_JMP	treat_instruction
;flash3:	bra	flash2
...
.OnTxt:	dc.b 'MMU & TTx switched on',0
.OffTxt:	dc.b 'MMU & TTx switched off',0
.ErrorTxt:dc.b 'No MMU',0
	even
;  #] MMU Stuff: 
;  #[ Context switching:
	; #[ Put_internal_sr:
...
berr_handler:
	_30
	btst	#0,$a(sp)		;SSW_DF
	beq.s	breakpt		;pas un acces DATA
	btst	#6,$b(sp)		;SSW_RW
	beq.s	breakpt		;acces ecriture
	cmp.l	#4,$10(sp)
	bne.s	breakpt		;pas un acces a execbase
	move.l	([internal_a6],ExecBase),$2c(sp)
	bclr	#0,$a(sp)		;clear RERUN
	rte
	_00
	ENDC	;A3000
	; #[ Breakpt:
...
p0p1:
	...
	beq.s	.no_40
	jsr	CacheClearU(a6)	;en attendant mieux...
.no_40:	move.l	(sp)+,a6
	bsr	test_task
	bpl.s	.task_ok
	rts
...
.afterm:	IFEQ	A4000
	...
	addq.w	#8,a0
	dc.w	$f010,$4000	;pmove.l	(a0),tc ;enforcer=#$80a08680
	addq.w	#4,a0
...
._40:	dc.l	$4e7a0803		;movec	msp,d0
...
treat_ptermerr:
	...
	bsr	clr_table_end
	IFNE	AMIGA
	move.l	before_ctrll_la_addr(a6),current_la_addr(a6)
	ENDC
	move.l	d0_buf(a6),-(sp)
	move.l	(sp),-(sp)
	move.l	(sp),-(sp)
...
init_edit_mac:
	...
	; #] Edit mac:
;  #] Mac stuff:
sprintf3:	_JMP	sprintf2
strclr3:	_JMP	strclr2
strcpy3:	_JMP	strcpy2
strlen3:	_JMP	strlen2
strcat3:	_JMP	strcat2
init_buffer3:	_JMP	init_buffer2
