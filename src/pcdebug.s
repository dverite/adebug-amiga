...
;flag ascii scroll
	IFNE	turbodebug
	IFNE	C_EVAL
;interface avec l'evaluateur
	XDEF	d0_buf,text_buf,data_buf,bss_buf,FindVar,_FindVar
	ENDC
;;		#[ Pure C:
;	 #[ tcget_sizeof:
;In:a1=struct @
...
.l1:	cmp.l	a4,a3
	...
	bsr	print_press_key
.check_size:
	cmp.l	DTA_SIZE+my_dta(a6),d7
	bge.s	.not_higher
	move.l	DTA_SIZE+my_dta(a6),d7
.not_higher:
.not_found:
	addq.w	#8,a3
	bra.s	.l1
...
.chck:	cmp.w	#4,d0		;trap/linea/linef/
	...
	blt.s	.branch
.rts:	IFNE	ATARIST
	add.l	text_buf(a6),d7	;rts/rte/rtr
	ENDC
	IFNE	AMIGA
	move.l	d7,d0
	bsr	Sectionize
