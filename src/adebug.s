...
ATARIST	EQU	0
ATARITT	EQU	0
MTOS	EQU	0	;pas en meme temps que cartouche
AMIGA	EQU	1
A3000	EQU	1
A4000	EQU	0
MAC	EQU	0
...
debug	EQU	0
	...
	IFNE	accessoire
	IFNE	debug
	_OUTPUT	c:\aadebugf.acc
	ELSEIF
	_OUTPUT	aadebugf.acc
	ENDC
	ENDC
...
;2->either, better is slow (unused, slow can't be better :-)
;3->either, better is fast
_reserve_memory:
	move.l	d3,-(sp)
	subq.w	#1,d1
...
.var:
	...
	lsl.l	#2,d1		;mulu	#TCVARS_SIZEOF,d1
	sub.l	d1,(a2)
	bra	redraw_curwind
.start_var:
...
;	beq	.end
	...
	lsl.l	#2,d0		;mulu	#TCVARS_SIZEOF,d0
	add.l	d0,(a0)
	bra	redraw_curwind
.end_of_var:
...
.delete3:
 clr.w 8(a0)
 move.w big_window_coords+2(a6),$12(a0)
 move.w big_window_coords+2(a6),$22(a0)
 IFNE	ATARIST
 add.w #$b,$16(a0)
 add.w #$b,$26(a0)
 ENDC
 IFNE	AMIGA
 move.w	alt_s_sub1(a6),d0
 add.w d0,$16(a0)
 add.w d0,$26(a0)
 ENDC
 st $19(a0)
...
; move.w #12,$12(a0)
; move.w #12,$22(a0)
 move.w	big_window_coords+2(a6),d0
 IFNE	ATARIST
 add.w	#11,d0
 ENDC
 IFNE	AMIGA
 add.w	alt_s_sub1(a6),d0
 ENDC
 move.w	d0,$12(a0)
 move.w	d0,$22(a0)
 IFNE	ATARIST
...
.8:
 move.w	w3len(a6),d1
 cmp.w $32(a0),d1
 bne.s .dont_schrink_w4
 subq.w #6,$32(a0)
 move.w #6,$36(a0)
.dont_schrink_w4:
 cmp.w $42(a0),d1
 bne.s .dont_schrink_w5
 subq.w #6,$42(a0)
 move.w #6,$46(a0)
.dont_schrink_w5:
 move.w #1,window_selected(a6)
...
.meta_rez_alt_s:
 ...
 move.w window_selected(a6),d0
 btst #0,d0
 beq .meta_disassemble
.meta_memory_dump:
 cmp.w #1,d0
 beq .meta_register_window
 tst.b $48(a0)
 beq .meta_create1
.meta_delete1:
 move.l #$0000ff00,(a1)
...
.meta_8:
 cmp.w #$33,$32(a0)
 bne.s .meta_dont_schrink_w4
 sub.w #$c,$32(a0)
 move.w #$c,$36(a0)
.meta_dont_schrink_w4:
 cmp.w #$33,$42(a0)
 bne.s .meta_dont_schrink_w5
 sub.w #$c,$42(a0)
 move.w #$c,$46(a0)
.meta_dont_schrink_w5:
 move.w #1,window_selected(a6)
 st relock_vars_flag(a6)
 bra .1
...
;	tst.w	source_type(a6)
;	beq.s	.no_source
	cmp.w	#SOURCE_PC,source_type(a6)	;daniel
	bne.s	.no_source					;daniel
	bsr	get_curwind_type
	beq.s	.no_source
	lea	line_buffer(a6),a0
	move.l	d3,d0
...
;  #] Ask expression:
	IFNE	C_EVAL
;  #[ Eval C expr:[Ctl_Alt_E]
eval_c_expr:
	movem.l	a0-a3/d0-d3,-(sp)
	cmp.w	#SOURCE_BSD,source_type(a6)
	bne.s	.end
	suba.l	a0,a0
	lea	ask_c_expr_text,a2
	st	no_eval_flag(a6)
	_JSR	get_expression
	bmi.s	.end
	sf	no_eval_flag(a6)
	move.l	bss_buf(a6),-(sp)
	move.l	data_buf(a6),-(sp)
	move.l	text_buf(a6),-(sp)
	pea	d0_buf(a6)
	move.l	l6_ptr(a6),-(sp)
	move.l	l5_ptr(a6),-(sp)
	move.l	l3_ptr(a6),-(sp)
	move.l	tcnames_ptr(a6),-(sp)
	move.l	l5_len(a6),-(sp)

	IFNE	ATARIST	;PureC + exactement
	XREF	EvalC
	move.l	sp,d0
	lea	line_buffer(a6),a1
	jsr	EvalC
	...
	XREF	_EvalC
	move.l	sp,a1
	move.l	a1,-(sp)		;struct SrcDbgPtrs
	pea	line_buffer(a6)	;resultat
	move.l	a0,-(sp)		;expr a evaluer
	jsr	_EvalC
	lea	(9+3)*4(sp),sp
	ENDC	;AMIGA
...
;break a l'instruction suivante
	...
	bsr.s	_ctrl_a
	beq	p0p1
.end:	sf	nowatch_flag(a6)
	rts

_ctrl_a:	st	runandbreak_flag(a6)
	move.l	pc_buf(a6),a0
	bsr	get_next_instruction
	bmi.s	__ctrla_error
	;bra	__ctrl_a
